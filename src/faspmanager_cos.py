...
def node_info(service_credential_file, bucket_region, bucket_name):
    ...
    response = requests.post(IBM_CLOUD_OAUTH_URL + '/token', data=token_req_data, headers={'Content-type': 'application/x-www-form-urlencoded'})
    if response.status_code != 200:
        raise Exception("error")
    ...
    header_auth = {
        'ibm-service-instance-id':service_credentials['resource_instance_id'],
    ...
    # 6- get delagated token to access the node api
