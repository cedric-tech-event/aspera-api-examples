...
# 	echo "CONFIG_TRSDK_ARCH=$$(sed -n 's/^    system_type: *//p' < config.yaml)" >> $(CONFIG_MK)
$(CONFIG_MK): config.yaml Makefile
	echo "# Generated by makefile, do not edit" > $(CONFIG_MK)
	echo "CONFIG_TMPDIR=$${TMPDIR:-$$(dirname $$(mktemp))}" >> $(CONFIG_MK)
	...
	case $$(uname -s)-$$(uname -m) in\
	  ...
	  *) arch=config_make_replace_architecture_from_sdk_here;echo "Cannot find architecture for $$(uname -s)-$$(iname -m). Edit $(CONFIG_MK) manually to set the value according to SDK.";;\
