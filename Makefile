...
.is_setup: $(CONFIG_TRSDK_DIR_ARCH)/asperatransferd $(CONFIG_TRSDK_CONFIG) $(CONFIG_SDK_ROOT)/aspera_ssh_bypass_rsa.pem
	touch $@
# detect os and architecture to get right binary
# 	echo "CONFIG_TRSDK_ARCH=$$(sed -n 's/^  arch: *//p' < config.yaml)" >> config.make
config.make: config.yaml Makefile
	echo "# Generated by makefile, do not edit" > config.make
	echo "CONFIG_TMPDIR=$${TMPDIR:-$$(dirname $$(mktemp))}" >> config.make
	echo "CONFIG_PRJ_ROOT=$$(pwd -P)" >> config.make
	echo 'CONFIG_YAML=$$(CONFIG_PRJ_ROOT)/config.yaml' >> config.make
